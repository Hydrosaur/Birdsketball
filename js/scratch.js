...
var Scratch = {
	...
	render: function(){
		...
		sprites.forEach(function(item){
			if(item.visible) {
				...
				Scratch.game.drawImage(document.querySelector("#images #" + item.image), item.x, item.y, item.width, item.height);
			}
		});
		// Scratch.text.forEach(function(item, id){
		// 	Scratch.game.fillStyle = item.color;
		// 	Scratch.game.font = item.css;
		// 	Scratch.game.fillText(item.text, item.x, item.y);
		// });
	},
	...
	drawText: function(css, text, color, x, y){
		Scratch.game.fillStyle = color;
		Scratch.game.font = css;
		Scratch.game.fillText(text, x, y);
	},
	init: function(elem){
...
function Sprite(image) {
	...
	this.y = 0;
	this.angle = 0;
	this.natwidth = document.querySelector("#images #" + this.image).naturalWidth;
	this.natheight = document.querySelector("#images #" + this.image).naturalHeight;
	...
	this.draw = function(){
		Scratch.game.drawImage(document.querySelector("#images #" + this.image), this.x, this.y, this.width, this.height);
	}
	...
	this.setSize = function(es){
		this.width = (es * 0.01) * this.natwidth;
		this.height = (es * 0.01) * this.natheight;
		if(Scratch.autorender) Scratch.render();
	}
	this.resize = function(ew,eh){
		this.width = ew;
		this.height = eh;
		if(Scratch.autorender) Scratch.render();
	}
